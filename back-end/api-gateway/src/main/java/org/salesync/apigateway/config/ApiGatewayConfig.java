...
public class ApiGatewayConfig {
    ...
    public RouteLocator gatewayRoutes(RouteLocatorBuilder builder) {
        return builder.routes()
                .route(Service.Auth.NAME,
                        r -> r.path(MessageFormat
                                        .format("{0}{1}/**", Service.CONTEXT_PATH, Service.Auth.ENDPOINT))
                                .filters(f -> f.rewritePath(MessageFormat.format("{0}{1}", Service.CONTEXT_PATH, Service.Auth.ENDPOINT), Service.Auth.AUTH_CONTEXT_PATH))
                                .uri(UriComponentsBuilder.fromUriString(authServerUrl)
                                        .build().toUriString())
                )
