...
public interface RecordRepository extends JpaRepository<Record, UUID> {
    @Query (value = "SELECT * FROM get_filtered_records(:userId, :name, :typeId, :searchTerm, :isAsc)",
    nativeQuery = true
    )
    Page<Record> getFilteredRecord(UUID userId, String name, UUID typeId, String searchTerm, boolean isAsc, Pageable pageable);
    @Query (value = "SELECT * FROM get_filtered_records_and_sort_by_name(:userId, :typeId, :searchTerm, :isAsc)",
            nativeQuery = true
    )
