...
public class RecordServiceImpl implements RecordService {
	...
	public ListRecordsResponseDto getFilteredRecords(ListRecordsRequestDto requestDto, String companyName) {
		...
		if(requestDto.getPropertyName() != null) {
			if(roles.contains("read-all")) {
				page = recordRepository.getAllFilteredRecord(
						UUID.fromString(SecurityContextHelper.getContextUserId()), requestDto.getPropertyName(),
						requestDto.getTypeId(), requestDto.getSearchTerm(), requestDto.isAsc(), pageRequest, companyName
				);
			...
			if(roles.contains("read-all")) {
				page = recordRepository.getAllFilteredRecordsAndOrderByName(
						UUID.fromString(SecurityContextHelper.getContextUserId()), requestDto.getTypeId(),
						requestDto.getSearchTerm(), requestDto.isAsc(), pageRequest, companyName
				);
			} else if(roles.contains("read-own")) {
		...
		final List<RecordDto> recordDtos =
				page != null ? page.getContent().stream().map(recordMapper::recordToRecordDto).toList()
						: Collections.emptyList();

		return ListRecordsResponseDto.builder()
				.records(recordDtos)
				.totalSize(Optional.ofNullable(page).map(Page::getTotalElements).orElse(0L))
				.pageSize(Optional.ofNullable(page).map(Page::getSize).orElse(0))
				.currentPage(Optional.ofNullable(page).map(Page::getNumber).orElse(0) + 1)
				.build();
	}

	@Override
	public List<RecordDto> getAllRecords(String companyName) {
		return recordRepository.findAllByCompanyName(companyName)
				.stream()
	...
	public ListRecordsResponseDto getAllRecordsWithCondition(ListRecordsRequestDto listRecordsRequestDto) {
		return ListRecordsResponseDto.builder()
				.records(recordRepository.findAll().stream().map(recordMapper::recordToRecordDto).toList())
				.build();
	}
	...
	public RecordTypePropertyDto updateRecordProperty(RecordTypePropertyDto recordTypePropertyDto) {
		RecordTypeProperty recordTypeProperty = recordTypePropertyRepository.findById(recordTypePropertyDto.getId())
				.orElseThrow(
						() -> new ObjectNotFoundException(
	...
	public void deleteRecordsById(List<UUID> recordIds) {
		...
		recordIds.forEach(recordId -> {
			...
			if(permissions.contains(PermissionType.DELETE_OWN.getPermission())) {
			...
			} else if(!permissions.contains(PermissionType.DELETE_ALL.getPermission())) {
				throw new AccessDeniedException("You are not allowed to delete this record");
			}
	...
	public ListRecordTypeRelationsDto getListRecordTypeRelationsById(UUID sourceRecordId, String token, String realm) {
		...
		ResponseEntity<List<TypeDto>> response = restTemplate.exchange(
				"http://type-service/api/v1/" + realm + "/types", HttpMethod.GET, entity,
	...
	@Override
	public RecordDto createRecordByTypeId(String companyName, String typeId, String token,
	...
	public RecordDto updateRecordByRecordId(String recordId, String token, RecordDto updateRecordRequestDto) {
		...
		String userId = tokenService.extractClaim(token.split(" ")[1], claims -> claims.get("userId", String.class));
		rabbitMQProducer.sendMessage("record", MessageDto.builder()
				.content("${" + userId + "} Updated " + recordEntity.getName())
				.title("Record Updated")
				.createdAt(new Date())
				.action("update")
				.isRead(false)
				.url("/record/" + recordEntity.getId())
				.senderId(UUID.fromString(userId))
				.receiverId(recordEntity.getUserId())
				.build());
	...
	@Override
	public List<RecordDto> createListRecord(String realm, String token,
			List<CreateRecordRequestDto> createRecordRequestDtos) {
