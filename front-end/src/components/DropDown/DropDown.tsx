...
const DropDown: React.FC<DropdownButtonProps> = ({ children, value, header, disabled, showHeader = true }) => {
  ...
  return (
    <div ref={buttonRef} className='dropdown relative' onKeyDown={handleKeyDown}>
      ...
      <div
        ref={menuRef}
        className={`divide absolute z-10 max-h-80
                divide-y-2 divide-button-stroke-light overflow-y-auto rounded border-2 border-button-stroke-light bg-button-background-light
                px-2 dark:divide-button-stroke-dark dark:border-button-stroke-dark dark:bg-button-background-dark ${
                  shoulDropUp ? 'top-[4.8rem] origin-top' : 'bottom-12 origin-bottom'
                }
                ${isOpen ? 'scale-100' : 'scale-0 *:hidden'} transition-all duration-100 ease-in-out`}
        onClick={(event) => {
          let target = event.target as HTMLElement;
          while (target && target !== event.currentTarget) {
            if (target.parentNode === event.currentTarget) {
