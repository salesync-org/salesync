...
const Modal = ({ children, title, isOpen, onClose, className, isStatic = true }: Props) => {
  ...
  return ReactDOM.createPortal(
    <>
      {isOpen && (
        <div
          className='fixed left-0 right-0 top-0 z-[199] flex h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-x-hidden overflow-y-hidden bg-black bg-opacity-50 transition-all dark:bg-gray-900 dark:bg-opacity-50 md:inset-0'
